/** Ported 1:1 from templates/auth/forgot_password.html. */
export function AuthForgotPasswordPage(props: { brandName: string; sent: boolean; error?: string | null }) {
  return (
    <html lang="en">
      <head>
        <meta charSet="UTF-8" />
        <meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover" />
        <title>Forgot password · {props.brandName}</title>
        <style>{`
    @import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@700&family=Share+Tech+Mono&display=swap');
    :root { --bg:#0a0a0a; --card:#111; --border:#1e1e1e; --accent:#00f5ff;
            --text:#e0e0e0; --dim:#666; --ok:#00e676; --err:#ff2d78; }
    * { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
    ...
    .ok-box { background:rgba(0,230,118,.08); border:1px solid rgba(0,230,118,.25);
              border-radius:8px; padding:14px 16px; color:var(--ok); font-size:14px; }
    .ok-box strong { display:block; margin-bottom:4px; }
