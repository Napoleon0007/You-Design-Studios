/** Ported 1:1 from templates/auth/login.html. */
export function AuthLoginPage(props: { brandName: string; error?: string | null; next?: string }) {
  return (
    <html lang="en">
      <head>
        <meta charSet="UTF-8" />
        <meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover" />
        <title>Sign in · {props.brandName}</title>
        <style>{`
      ...
      <body>
        ...
        <div class="card corner">
          ...
          <div class="links">
            ...
            <a href={`/auth/register${props.next ? `?next=${props.next}` : ""}`}>Create account</a>
          </div>
        </div>
      </body>
    </html>
