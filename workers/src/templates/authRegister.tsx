/** Ported 1:1 from templates/auth/register.html. */
export function AuthRegisterPage(props: { brandName: string; error?: string | null; next?: string }) {
  return (
    <html lang="en">
      <head>
        <meta charSet="UTF-8" />
        <meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover" />
        <title>Create account · {props.brandName}</title>
        <style>{`
    ...
    .links a:hover { text-decoration:underline; }
      ...
      <body>
        ...
        <div class="card corner">
          ...
          <p class="sub">Design. Order. Track.</p>
          ...
          <div class="links">
            ...
            <a href={`/auth/login${props.next ? `?next=${props.next}` : ""}`}>Sign in</a>
          </div>
        </div>
      </body>
    </html>
